    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($table == 'barang_masuk') {
                $query = $this->User_model->getBarangMasuk(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            } else {
                $query = $this->User_model->getBarangKeluar(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            }
    ...
    private function _cetak($data, $table_, $tanggal)
    {
        ...
        if ($table_ == 'barang_masuk') :
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(95, 7, $d['nama'], 1, 0, 'L');
                $pdf->Cell(30, 7, $d['jumlah'], 1, 0, 'C');
                $pdf->Ln();
            }
        endif;
